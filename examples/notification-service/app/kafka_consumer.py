...
Uses the gRPC client to look up user details when processing events,
rather than the HTTP user-service endpoint. This is the common pattern
in polyglot microservice architectures where internal service-to-service
calls go over gRPC while public APIs use HTTP.
"""
import asyncio
import json
import logging
...
from app.grpc_client import get_user
...
async def handle_todo_created(event_data, db_session=None):
    ...
    2. Fetch user details via gRPC (users.UserService/GetUser)
    ...
    try:
        ...
        # Fetch user details via gRPC instead of HTTP
        user = await get_user(user_id)
        if not user:
            logger.error(f"User not found via gRPC: {user_id}")
            return
