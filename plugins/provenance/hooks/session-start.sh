...
#
# `linespec provenance next` also re-derives the fswrite write-bit projection
# (prov-2026-8d2f5f2a) from the current set of open records' scopes on every
# invocation, unconditionally — not gated behind anything this script sets.
# Enforcement must not depend on this hook being installed at all (a user may
# not have Claude Code hooks set up), so this script is purely a renderer of
# `next`'s output; a dedicated `linespec provenance reconcile` verb also exists
# for explicit/scripted invocation outside any hook.
