...
function generateDeleteSql(table, payload) {
    ...
    throw new Error(`Cannot generate DELETE for table "${table}" - payload must include an 'id' field to identify which record to delete.\n` +
...
function inferColumnsFromOtherExpects(spec, payloads, currentTable) {
    ...
    for (const expect of spec.expects) {
        ...
        if (!('rows' in parsed)) {
            return generateColumnsFromRow(parsed, currentTable);
        }
    }
    return null;
}
// Generate column definitions from a row object
function generateColumnsFromRow(rowData, tableName) {
    let seqId = 2;
...
function generateEmptyResultResponse(tableName, columns) {
    ...
    for (let i = 0; i < spec.expects.length; i++) {
        ...
        if (expect.channel === 'READ_MYSQL' || expect.channel === 'WRITE_MYSQL') {
            ...
            if (expect.sql) {
                ...
                const writeExpect = expect;
                if (writeExpect.operation) {
                    // Explicit operation type specified
                    switch (writeExpect.operation) {
                        case 'DELETE':
                            if (!withFile) {
                                throw new Error(`WRITE:MYSQL DELETE for table "${expect.table}" requires a WITH file to identify which record to delete.\n` +
                    ...
                    throw new Error(`WRITE:MYSQL for table "${expect.table}" requires either a WITH file or an explicit operation type (INSERT, UPDATE, or DELETE).\n` +
                        `Example: EXPECT WRITE:MYSQL DELETE ${expect.table} WITH {{payloads/delete_request.yaml}}`);
                }
            }
            ...
            const returnsEmpty = expect.returnsEmpty;
            if (returnsEmpty) {
