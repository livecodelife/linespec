...
`;
            ...
            await spawnProcess('cp', [path.join(__dirname, '..', 'package.json'), path.join(proxyBuildDir, 'package.json')]);
            try {
                await spawnProcess('cp', [path.join(__dirname, '..', 'package-lock.json'), path.join(proxyBuildDir, 'package-lock.json')]);
            }
            catch {
                // ignore
            }
            await spawnProcess('cp', [mocksPath, path.join(proxyBuildDir, 'mocks.yaml')]);
            ...
            for (const file of distFiles) {
                ...
                if (stat.isFile()) {
                    await spawnProcess('cp', [srcPath, path.join(proxyBuildDir, 'dist', file)]);
                }
            }
            await spawnProcess('docker', [
