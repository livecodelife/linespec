...
async function main() {
    ...
    const { server, mgmtServer, state } = await (0, mysql_proxy_1.startProxy)(docs, options.upstreamHost, options.upstreamPort, options.listenPort);
    console.error(`MySQL proxy listening on port ${options.listenPort} -> ${options.upstreamHost}:${options.upstreamPort}`);
    console.error(`Management server listening on port ${options.listenPort + 1}`);
