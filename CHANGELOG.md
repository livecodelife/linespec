...
- **Homebrew installs never produced a working `linespec:latest` image** ([prov-2026-f57f1570](./provenance/prov-2026-f57f1570.yml)) — the formula's `post_install` hook could not work on either platform, and both failure modes reached users. On macOS it always failed: `linespec build` needs a linespec source checkout, which a Cellar install does not have, and the formula misreported every occurrence as "Docker may not be running". On Linux it failed silently and worse — the hook staged the glibc-linked release binary into an Alpine (musl) base, producing an image whose binary could not exec, while `docker build` still exited 0. Neither was a regression in any single release; the hook had been broken since `go-tree-sitter` entered the dependency graph, and 3.19.0 shipped with it still in place. The hook is removed and the image is published from CI instead.
...
- **`provenance run-specs`/`complete` no longer pay a redundant framework boot per spec entry** ([prov-2026-3ee1f3c3](./provenance/prov-2026-3ee1f3c3.yml)) — consecutive `associated_specs` entries that resolve to the same effective command (same auto-run type, or the same `run_command` with no `{{path}}` placeholder) are now grouped into a single invocation with all their paths appended, instead of one process per entry — measured ~1.7x faster on a 3-spec record (#200).
- **Record-internal paths now resolve against the same base as `provenance.dir`** ([prov-2026-2f2bf9c3](./provenance/prov-2026-2f2bf9c3.yml)) — `affected_scope`, `forbidden_scope`, and `associated_specs` paths were previously resolved against the process cwd instead of the config-relative base `provenance.dir` uses, so linting (or opening) from inside a subpackage reported phantom missing-file failures, including a live record misreported as dead — fixed (#201).
- **`provenance complete` now actually runs a record's own `associated_specs` when `run_associated_specs_on_complete` is enabled** ([prov-2026-8d5c9376](./provenance/prov-2026-8d5c9376.yml)) — previously it only checked that each spec's `path` existed on disk, so a failing proof command was reported as verified and the record sealed anyway; `complete` now runs the specs and rolls back the transition (leaving the record `open`) on failure, and reports each spec's actual pass/fail outcome instead of mere file existence (#202).
- **`write_restriction` no longer locks a nested package's own `.linespec.yml`** ([prov-2026-c65450b6](./provenance/prov-2026-c65450b6.yml)) — the always-writable exemption for the config file that turns the restriction on was only ever derived for the repo root; it's now derived the same way as the provenance-directory exemption, so each config's own `.linespec.yml` stays writable relative to where it actually lives (#203).
- **`provenance index`/`search`/`audit`/`complete` now work with non-2048-dim local embedders** ([prov-2026-0f3c349b](./provenance/prov-2026-0f3c349b.yml), [prov-2026-9eefeb1b](./provenance/prov-2026-9eefeb1b.yml), [prov-2026-e1c1aa88](./provenance/prov-2026-e1c1aa88.yml)) — the embedding store no longer hardcodes a 2048-wide vector (voyage-4-large/voyage-4-lite's width); the width is now derived from the first vector actually written and persisted in `.linespec/embeddings.bin`'s self-describing header, so `openai`-provider `base_url` servers producing other widths (e.g. `nomic-embed-text-v1.5` at 768, `bge-large` at 1024, `all-MiniLM-L6-v2` at 384) work end to end. Existing headerless indexes keep working, since they were always written at 2048. Also fixed `index`'s summary line printing a `✓` when every record failed to index (0 succeeded); it now prints `✗` in that case (the non-zero exit code was already correct) (#204, #208).
- **`provenance index` no longer wedges the embedding store on a record id with a disambiguating suffix** ([prov-2026-772101a3](./provenance/prov-2026-772101a3.yml)) — `Store.ReadAll` read each record ID with a single `bufio.Reader.Read` instead of `io.ReadFull`, so a record ID of different length than its neighbors could trigger a short read that desynced parsing for every subsequent record in the file; a wedged store was then silently reported as fully indexed. Fixed to fully consume each ID and to surface (rather than mask) records whose existence check errored (#207, #209).
- **`provenance index`'s `-c`/`--config` now drives record discovery, not just embedding settings** ([prov-2026-531a0e0b](./provenance/prov-2026-531a0e0b.yml)) — follow-up to #189/prov-2026-57aff9e1: `filepath.Join` silently concatenated an absolute custom `provenance.dir` onto the `-c` config's directory instead of replacing it, producing a syntactically absolute but nonexistent directory; the loader treated the missing directory as "zero records" rather than an error, so indexing silently found nothing and reported success. Fixed (#205, #210).
...
- **PostgreSQL: async/`JOB` expectations now marked satisfied, and mocked result sets carry real column OIDs** ([prov-2026-a0bac0d9](./provenance/prov-2026-a0bac0d9.yml)) — an `EXPECT` matched via the simple-query (`Q`) protocol path is now recorded as a hit so async `RECEIVE JOB`/`EVENT` specs complete instead of timing out; the PostgreSQL proxy gained schema-loading wiring (mirroring the existing MySQL `--schema-file` path) so a mocked `RowDescription` with `RETURNS` no longer sends OID 0 for every column. Also fixes a missing `database.port` default when `database.image` is set explicitly, and a leaked `linespec-shared-net` Docker network after a failed run (#152).
- **`discover`'s framework-detected path no longer skips non-route directories** ([prov-2026-3486daec](./provenance/prov-2026-3486daec.yml)) — once a framework (chi, rails, sinatra) is detected, the framework-agnostic supplemental pass now still runs for directories with no route registrations, so services, repositories, models, and utilities get blueprint coverage instead of only route-bearing files.
...
- **`linespec provenance discover`** ([prov-2026-c60ec2ad](./provenance/prov-2026-c60ec2ad.yml), [prov-2026-95b526f5](./provenance/prov-2026-95b526f5.yml), [prov-2026-da18cbfe](./provenance/prov-2026-da18cbfe.yml), [prov-2026-2042c6a2](./provenance/prov-2026-2042c6a2.yml), [prov-2026-a18e2104](./provenance/prov-2026-a18e2104.yml), [prov-2026-010bb9ec](./provenance/prov-2026-010bb9ec.yml)) — scans a target repository's source and auto-generates draft blueprint provenance records plus `.linespec` stubs from its discovered HTTP routes and cross-service protocol boundaries: a zero-to-governed on-ramp for existing codebases. Built in phases: tree-sitter parsing infrastructure and a framework-description loader, introducing a CGO dependency via `go-tree-sitter` ([prov-2026-010bb9ec](./provenance/prov-2026-010bb9ec.yml), [prov-2026-ac156eb7](./provenance/prov-2026-ac156eb7.yml)); route discovery and framework descriptions for **Chi** (Go), **Rails**, and **Sinatra** (Ruby) with a position-aware query engine and route-table assembler ([prov-2026-c60ec2ad](./provenance/prov-2026-c60ec2ad.yml), [prov-2026-9d655373](./provenance/prov-2026-9d655373.yml), [prov-2026-f6e3c9c2](./provenance/prov-2026-f6e3c9c2.yml)); protocol-boundary tracing via a syntactic call-graph walk ([prov-2026-95b526f5](./provenance/prov-2026-95b526f5.yml), [prov-2026-e565ad85](./provenance/prov-2026-e565ad85.yml)); `.linespec` stub generation with a dry-run plan and table/JSON output ([prov-2026-da18cbfe](./provenance/prov-2026-da18cbfe.yml), [prov-2026-db4734d9](./provenance/prov-2026-db4734d9.yml), [prov-2026-2d91c2ba](./provenance/prov-2026-2d91c2ba.yml)); blueprint-record generation and CLI wiring with `--dir` scoping and a `linespecs/` output directory ([prov-2026-2042c6a2](./provenance/prov-2026-2042c6a2.yml), [prov-2026-e49a5c61](./provenance/prov-2026-e49a5c61.yml), [prov-2026-a9a09e43](./provenance/prov-2026-a9a09e43.yml)); and `--enrich` — git-history + LLM intent synthesis to populate the `intent` field, with `--llm-url` for a local LLM server, per-record progress logging, and target-repo author inference ([prov-2026-a18e2104](./provenance/prov-2026-a18e2104.yml), [prov-2026-0d713fa1](./provenance/prov-2026-0d713fa1.yml), [prov-2026-3d5fb504](./provenance/prov-2026-3d5fb504.yml), [prov-2026-9d212b39](./provenance/prov-2026-9d212b39.yml), [prov-2026-cd3af872](./provenance/prov-2026-cd3af872.yml), [prov-2026-f3df5dfc](./provenance/prov-2026-f3df5dfc.yml)). See [`LINESPEC_DISCOVER.md`](./LINESPEC_DISCOVER.md).

- **`exclude_paths` provenance config** ([prov-2026-1b5fa2c2](./provenance/prov-2026-1b5fa2c2.yml), [prov-2026-520279fa](./provenance/prov-2026-520279fa.yml)) — a glob/full-path/directory-prefix/regex list in the `provenance` section of `.linespec.yml` that fully exempts matching files (generated code, documentation, migration scripts, tooling configs) from governance: the linter skips them, the commit checker skips both scope-enforcement and the commit-tag requirement, and `context` surfaces the exemption status explicitly rather than silently omitting it.

- **Auto-generated CLI reference** ([prov-2026-004a7d33](./provenance/prov-2026-004a7d33.yml)) — the full command/flag reference is now generated from the Cobra command tree via `cobra/doc` and wired into `scripts/build-docs.sh` (`docs/cli.html`), so the published reference can no longer drift from the code.

### Changed

- **CLI migrated to a declarative Cobra command tree** ([prov-2026-f1241113](./provenance/prov-2026-f1241113.yml), and imprints [prov-2026-a8fe26ea](./provenance/prov-2026-a8fe26ea.yml), [prov-2026-334ced6a](./provenance/prov-2026-334ced6a.yml), [prov-2026-c226f230](./provenance/prov-2026-c226f230.yml), [prov-2026-ede333d1](./provenance/prov-2026-ede333d1.yml), [prov-2026-af7becf2](./provenance/prov-2026-af7becf2.yml)) — the hand-rolled `os.Args`/switch dispatch in `cmd/linespec/` is replaced by a Cobra tree that wraps the existing handlers as `RunE` callbacks with no logic change, and the two build-tagged entrypoints (`main_stable.go`, `main_beta.go`) are collapsed into a single tree (`cli.go`). The command/flag surface is now declarative, so the CLI reference auto-generates from it. `make build-beta` / `make install-beta` remain backward-compatible aliases producing the identical binary.

- **Docs site is now built from Markdown source in CI** ([prov-2026-a7b20513](./provenance/prov-2026-a7b20513.yml), [prov-2026-5ff0f002](./provenance/prov-2026-5ff0f002.yml), [prov-2026-13576bee](./provenance/prov-2026-13576bee.yml)) — `LINESPEC.md` and `PROVENANCE_RECORDS.md` are the single source of truth; `scripts/build-docs.sh` renders the site with pandoc plus a Lua filter (preserving the dark theme, sidebar nav, and code-block copy buttons), and the Pages workflow rebuilds on every push to `main`. All rendered `docs/*.html` and `docs/llms*.txt` are now **gitignored build artifacts** — never hand-edit them. The docs-update process is documented in `CLAUDE.md` and `README.md`, and `scripts/bump-version.sh` now also rewrites the `version-<ver>` badge and `linespec_<ver>` artifact-name patterns so they cannot drift.

- **Consolidated duplicated logic flagged by the `dupl` gate** ([prov-2026-23ab1db8](./provenance/prov-2026-23ab1db8.yml), [prov-2026-4bab0511](./provenance/prov-2026-4bab0511.yml), [prov-2026-b5c9c813](./provenance/prov-2026-b5c9c813.yml)) — no behavior change. A single generic `verifyWithExtractor` helper now backs `VerifyHTTP`/`VerifyKafka`/`VerifyGRPC`/`VerifyRedis`, and a shared JSON body matcher (`pkg/verify/bodymatch.go`) is used by both the HTTP and Kafka interceptors ([prov-2026-875617fa](./provenance/prov-2026-875617fa.yml), [prov-2026-e15c3bd6](./provenance/prov-2026-e15c3bd6.yml)); the `findMock`/`peekMock` pairs in the MySQL and PostgreSQL proxies are collapsed into one hit-count-parameterized implementation and `waitForMySQL`/`waitForPostgreSQL` share a `waitForSQLDB` helper ([prov-2026-b1c025a5](./provenance/prov-2026-b1c025a5.yml), [prov-2026-c30ae0cb](./provenance/prov-2026-c30ae0cb.yml)); plus smaller `extractRows` (MySQL proxy) and `errEmbeddingNotConfigured` (provenance commands) extractions ([prov-2026-7b58bc99](./provenance/prov-2026-7b58bc99.yml), [prov-2026-29c68262](./provenance/prov-2026-29c68262.yml)).

### Fixed

- **`Dockerfile.linespec` cgo build broken by the unified binary** ([prov-2026-80db53da](./provenance/prov-2026-80db53da.yml)) — after the Cobra collapse the single `cmd/linespec` entrypoint imports `pkg/discover` unconditionally, pulling in the cgo package `go-tree-sitter`; the `golang:*-alpine` image built with no C toolchain and CGO effectively disabled failed with `undefined: Node`. The image now installs `build-base` and builds with `CGO_ENABLED=1`, and drops the now-legacy `-tags beta` (nothing is gated on the beta build tag after the collapse — the default build is the unified superset).

- **README version-badge drift** ([prov-2026-13576bee](./provenance/prov-2026-13576bee.yml)) — the shields.io version badge and the release-artifact filename example were stuck two releases behind `VERSION` because `bump-version.sh` only rewrote `v<ver>` patterns; both are corrected and future-proofed by the `bump-version.sh` change above.
