...
function buildKMocks(spec: TestSpec, payloads: Map<string, PayloadFile>): KMock[] {
  ...
  if (hasMysqlExpect) {
    const setNamesMock: KMock = {
      version: 'api.keploy.io/v1beta1',
      kind: 'MySQL',
      name: `${spec.name}-mock-set-names`,
      spec: {
        metadata: {
          connID: '0',
        ...
        requests: [
          {
            ...
            message: {
              query: 'SET NAMES utf8mb4,  @@SESSION.sql_mode = CONCAT(CONCAT(@@sql_mode, \',STRICT_ALL_TABLES\'), \',NO_AUTO_VALUE_ON_ZERO\'),  @@SESSION.wait_timeout = 2147483',
            },
          },
        ],
        responses: [
    ...
    mocks.push(setNamesMock);

    const comPingMock: KMock = {
      version: 'api.keploy.io/v1beta1',
      kind: 'MySQL',
      name: `${spec.name}-mock-ping`,
      spec: {
        metadata: {
          connID: '0',
          requestOperation: 'COM_PING',
          responseOperation: 'OK',
          type: 'mocks',
        },
        requests: [
    ...
    mocks.push(comPingMock);

    const schemaMock: KMock = {
      version: 'api.keploy.io/v1beta1',
      kind: 'MySQL',
      name: `${spec.name}-mock-schema`,
      spec: {
        metadata: {
          connID: '0',
          requestOperation: 'COM_QUERY',
          responseOperation: 'TextResultSet',
          type: 'mocks',
        },
        requests: [
          {
            ...
            message: {
              query: 'SELECT table_name FROM information_schema.tables WHERE table_schema = database() AND table_name = \'schema_migrations\' AND table_name IN (SELECT table_name FROM information_schema.tables WHERE table_schema = database())',
            },
          },
        ],
        responses: [
          {
            ...
            message: {
              ...
              eofAfterColumns: [5, 0, 0, 3, 254, 0, 0, 2, 0],
              rows: [
                {
                  header: {
                    payload_length: 18,
    ...
    mocks.push(schemaMock);

    const versionMock: KMock = {
      version: 'api.keploy.io/v1beta1',
      kind: 'MySQL',
      name: `${spec.name}-mock-versions`,
      spec: {
        metadata: {
          connID: '0',
          requestOperation: 'COM_QUERY',
        ...
        requests: [
          {
            ...
            message: {
              query: 'SELECT `schema_migrations`.`version` FROM `schema_migrations` ORDER BY `schema_migrations`.`version` ASC',
            },
          },
        ],
        responses: [
