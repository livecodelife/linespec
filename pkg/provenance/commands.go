...
// InstallSkillOptions holds options for the install-skill command
type InstallSkillOptions struct {
	Name string // Skill name (default: provenance)
	Path string // Target directory relative to repo root (default: .claude/skills)
}

// InstallSkill copies the provenance skill into a Claude Code skills directory.
func (c *Commands) InstallSkill(opts InstallSkillOptions) error {
	name := opts.Name
	if name == "" {
		name = "provenance"
	}
	...
	destDir := filepath.Join(c.RepoRoot, targetDir, name)
	if _, err := os.Stat(destDir); err == nil {
		return fmt.Errorf("destination already exists: %s\nRemove it first or choose a different name", destDir)
	}
	...
	content := provenanceSkillMD
	if name != "provenance" {
		content = strings.ReplaceAll(content, "name: provenance", "name: "+name)
	}
	...
	fmt.Fprintf(os.Stdout, "  Invoke it in Claude Code with: /%s\n\n", name)
